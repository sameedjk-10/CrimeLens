...
export const searchCrimes = async (req, res) => {
  try {
    ...
    if (["admin", "police"].includes(role)) {
      ...
      if (officer_name)
        whereClause["$User.name$"] = { [Op.iLike]: `%${officer_name}%` };
      if (person_name)
        whereClause["$Criminal.name$"] = { [Op.iLike]: `%${person_name}%` };
    }

    // 🚫 Restrict Public users from sensitive filters
    if (
      role === "public" &&
      (case_id || officer_id || officer_name || person_name)
    ) {
    ...
    if (!submitterRecord) {
      ...
      if (fullName || contact) {
        await submitterRecord.update({
          ...(fullName && { sumitteRName: fullName }),
...
export const updateCrime = async (req, res) => {
  try {
    ...
    if (!crime) {
      return res
        .status(404)
        .json({ success: false, message: "Crime not found" });
    }
...
export const deleteCrime = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Crime deleted successfully" });
  } catch (error) {
